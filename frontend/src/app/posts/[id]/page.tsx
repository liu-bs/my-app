...
/**
 * 预生成已发布文章的静态参数（ISR）
 * @description 构建时预渲染已发布文章，配合 revalidate 实现增量静态再生
 */
export async function generateStaticParams() {
  try {
    const data = await blogApi.listPosts({ page: 1, limit: 50 });
    return data.posts.map((p) => ({ id: p.id }));
  } catch {
    return [];
  }
}
      ...
      <div className="grid grid-cols-1 gap-10 pb-12 max-lg:gap-0 max-lg:pb-8 lg:grid-cols-[1fr_220px]">
        ...
        <article>
          ...
          <header className="article-head anim-fade-up stagger-1 mb-8">
            ...
            {isAuthor && <DeletePostButton postId={post.id} redirectTo="/posts" />}
          </header>
          ...
          <div id="article-content" className="article-content-wrapper anim-fade-up stagger-3">
            ...
            <LazyComments postId={post.id} user={user} />
          </div>
