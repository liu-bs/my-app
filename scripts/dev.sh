...
info "检查端口占用..."
check_port "$BACKEND_PORT" "后端" || fail "后端端口 ${BACKEND_PORT} 被占用，请先释放"
check_port "$FRONTEND_PORT" "前端" || fail "前端端口 ${FRONTEND_PORT} 被占用，请先释放"
ok "端口 ${BACKEND_PORT} 和 ${FRONTEND_PORT} 均可用"
...
if [[ ! -d "$ROOT_DIR/backend/node_modules" ]]; then
  warn "backend/node_modules 不存在，正在创建链接..."
  NODE_ENV=development pnpm install
fi

# 检查关键依赖是否可解析
if ! node -e "require.resolve('concurrently')" 2>/dev/null; then
  warn "concurrently 未安装，正在安装依赖..."
  NODE_ENV=development pnpm install
fi
...
echo -e "${GREEN}  后端: http://localhost:${BACKEND_PORT}/api${NC}"
